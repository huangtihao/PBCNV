...
bam = "/media/hth/Huang Tihao/data/lcdx/1000genomes/chr21/NA19238.chrom21.SLX.maq.SRP000032.2009_07.bam";
binSize = 1000
...
myin = open('/media/hth/Huang Tihao/data/lcdx/1000genomes/chr21/groundtruth/NA19238.txt','r')
line = myin.readline()
list1 = []
while line :
    a = line.split()
...
samfile=pysam.AlignmentFile("/media/hth/Huang Tihao/data/lcdx/1000genomes/chr21/NA19238.chrom21.SLX.maq.SRP000032.2009_07.bam", "rb")
map_q = []
pos_q = []
for r in samfile:
    map_q.append(r.mapq)
...
myOut2 = open('/media/hth/Huang Tihao/data/lcdx/1000genomes/chr21/groundtruth/NA19238_trains.txt','w')
...
for i in range(len(pos)):
    ...
    # # label     3:正常；1：gain；
    for j in range(list3.shape[0]):
        if (list3[j][0] <= pos[i] <= list3[j][1] and list3[j][2]==1 ):
            myOut2.write("\t" + "1" + "\n")
            break
        elif (list3[j][0] <= pos[i] <= list3[j][1] and list3[j][2]==2 ) :
            myOut2.write("\t" + "2" + "\n")
            break
        else:
            if j == (list3.shape[0] - 1):
